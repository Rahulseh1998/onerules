...
const WEAK_PATTERNS: { pattern: RegExp; message: string; suggestion: string }[] = [
  ...
  { pattern: /write tests/i, message: "\"Write tests\" without guidance leads to useless tests", suggestion: "Replace with: \"Test behavior not implementation. No mocks unless the real thing has side effects. Test failure modes.\"" },
  ...
  { pattern: /use typescript/i, message: "Just saying \"use TypeScript\" doesn't prevent AI from using `any` everywhere", suggestion: "Replace with: \"`any` is a bug. Use `unknown` with type guards. Infer types when TypeScript can.\"" },
